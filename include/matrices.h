...
// ===========================================================================
//  Operações vetoriais/matriciais elementares implementadas no projeto, para
//  não depender das funções equivalentes da GLM (vedadas pelo enunciado).
//  Nomes distintos dos da GLM (crossproduct/normalized/Matrix_Inverse) para
//  evitar ambiguidade por ADL ao receberem tipos glm::vec*/glm::mat*.
// ===========================================================================
...
// Norma Euclidiana de um vetor 3D.
inline float norm(glm::vec3 v)
{
    return sqrt(v.x*v.x + v.y*v.y + v.z*v.z);
}

// Versor (vetor unitário) de um vetor 3D.
inline glm::vec3 normalized(glm::vec3 v)
{
    return v / norm(v);
}

// Produto escalar entre dois vetores 2D / 3D (sem a restrição de w==0 imposta
// por dotproduct(glm::vec4,...)).
inline float dotproduct(glm::vec2 u, glm::vec2 v)
{
...
inline float distancePoints(glm::vec3 a, glm::vec3 b)
{
    return norm(a - b);
}

// Mínimo / máximo componente a componente de dois vetores 3D.
inline glm::vec3 Vector_Min(glm::vec3 a, glm::vec3 b)
{
    return glm::vec3(a.x < b.x ? a.x : b.x,
                     a.y < b.y ? a.y : b.y,
                     a.z < b.z ? a.z : b.z);
}
inline glm::vec3 Vector_Max(glm::vec3 a, glm::vec3 b)
{
    return glm::vec3(a.x > b.x ? a.x : b.x,
                     a.y > b.y ? a.y : b.y,
                     a.z > b.z ? a.z : b.z);
...
// Limita um escalar ao intervalo [lo, hi].
...
// Inversa de uma matriz 4x4 (cofatores / adjugada dividida pelo determinante).
// Equivalente a glm::inverse. Devolve a identidade se a matriz for singular.
inline glm::mat4 Matrix_Inverse(glm::mat4 M)
{
    // Cópia linear em ordem column-major (a mesma de OpenGL/GLM): m[col*4 + row].
    float m[16];
    for (int c = 0; c < 4; ++c)
        for (int r = 0; r < 4; ++r)
            m[c*4 + r] = M[c][r];
